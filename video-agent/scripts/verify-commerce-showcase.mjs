...
try{
 for(const c of cases){
  ...
  for(const scene of document.scenes){
   ...
   const check=await page.evaluate(id=>{const root=document.getElementById(id);return [...root.querySelectorAll('.line,.note,.cta,.foot')].flatMap(el=>{const r=el.getBoundingClientRect();return r.left<-.5||r.right>1080.5||r.top<-.5||r.bottom>1920.5||el.scrollWidth>el.clientWidth+2?[{text:el.textContent,rect:{x:r.x,y:r.y,w:r.width,h:r.height},scrollWidth:el.scrollWidth,clientWidth:el.clientWidth}]:[];});},scene.id);
   issues.push(...check.map(x=>({sceneId:scene.id,time:t,...x})));
   ...
   thumbs.push(await sharp(jpeg).resize(270,480).toBuffer());samples.push({sceneId:scene.id,time:t});
  }
  ...
  const firstScene=document.scenes[0];await seek(.8);const m1=await page.$eval(`#${firstScene.id} .photo-inner`,e=>getComputedStyle(e).transform);await seek(1.7);const m2=await page.$eval(`#${firstScene.id} .photo-inner`,e=>getComputedStyle(e).transform);
  const result={kind:'reference-author-browser-check',notLiveAgentEvidence:true,runtimeErrors,layoutIssues:issues,repeatableSeek:repeatable,productPhotoMotion:m1!==m2,motionSamples:[m1,m2],samples,fullVideoHumanReview:false,commercialRightsCleared:false};
  await fs.writeFile(path.join(ROOT,c.id,'qa.json'),JSON.stringify(result,null,2));
  await sharp({create:{width:270*5,height:480,channels:3,background:'#dddddd'}}).composite(thumbs.map((input,i)=>({input,left:i*270,top:0}))).jpeg({quality:91}).toFile(path.join(ROOT,c.id,'storyboard.jpg'));
  ...
  console.log('PASS '+c.id+' typography, product motion, repeated seek');await page.close();
 }
