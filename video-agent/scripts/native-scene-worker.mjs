 ...
 for(const scene of config.scenes){
  ...
  for(const time of times){
   const sample=await page.evaluate(async({time,targets,runtimeMedia})=>{
    const effectiveOpacity=element=>{let opacity=1;for(let current=element;current;current=current.parentElement)opacity*=Number(getComputedStyle(current).opacity);return opacity;};
    const seekRuntime=async()=>{const player=window.__player,hf=window.__hf;if(player?.renderSeek)player.renderSeek(time);else if(hf?.seek)hf.seek(time);else throw Error('媒体运行时未就绪');await window.__hfWaitForSeekCompletion?.();};
    if(runtimeMedia)await seekRuntime();
    else {
     const timeline=window.__timelines['commerce-root'];
     // A fresh paused GSAP timeline at zero has not rendered its zero-time
     // sets. Prime and return to zero before measuring the first frame.
     if(time===0&&timeline.time()===0)timeline.seek(0.000001,false);
     timeline.seek(time,false);
    }
    const media=[];
    if(runtimeMedia)for(const video of document.querySelectorAll('video')){const start=Number(video.dataset.start||0),duration=Number(video.dataset.duration);if(time<start||time>=start+duration)continue;const expected=Number(video.dataset.mediaStart||0)+(time-start)*Number(video.dataset.playbackRate||1),deadline=performance.now()+12000;let retryAt=performance.now()+250,retries=0;while(video.seeking||video.readyState<2||Math.abs(video.currentTime-expected)>1/30+.003){if(video.error)throw Error('媒体解码失败：'+video.id);if(performance.now()>deadline)throw Error('媒体未在目标帧就绪：'+video.id+' expected='+expected.toFixed(4)+' actual='+Number(video.currentTime).toFixed(4)+' ready='+video.readyState+' network='+video.networkState+' seeking='+video.seeking+' retries='+retries);if(!video.seeking&&performance.now()>=retryAt){await seekRuntime();retryAt=performance.now()+250;retries++;}await new Promise(r=>setTimeout(r,15));}media.push({id:video.id,currentTime:video.currentTime,expected,retries});}
