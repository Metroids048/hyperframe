...
await check('未配置模型返回明确失败并保留对话输入',async()=>{const n=p.revisions.length,j=await api(`/api/edit-projects/${p.id}/messages`,{baseRevisionId:p.currentRevisionId,text:'请保留开箱和产品特写'},202);const done=await wait(p.id,j.jobId);assert.equal(done.status,'failed');assert.match(done.error,/模型连接尚未就绪|云端模型尚未配置/);p=await api('/api/edit-projects/'+p.id);assert.equal(p.revisions.length,n);assert(p.messages.some(m=>m.text==='请保留开箱和产品特写'));});
await check('真实浏览器预览与 5～8 秒字幕逐帧边界',async()=>{
browser=await puppeteer.launch({executablePath:'/Users/a1234/Desktop/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true,defaultViewport:{width:1440,height:1000},args:['--disable-dev-shm-usage']});const page=await browser.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));
await page.goto(base+'/?project='+p.id,{waitUntil:'domcontentloaded'});await page.waitForSelector('#player',{timeout:30000});await sleep(500);
