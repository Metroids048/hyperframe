...
finally{await service.close?.();await fs.writeFile(path.join(out,mode+'-report.json'),JSON.stringify({mode,startedAt,endedAt:new Date().toISOString(),projectId:p?.id,source:'tears-of-steel.mp4',sourceSha256:JSON.parse(await fs.readFile(path.join(ROOT,'assets/edit-samples/tears-of-steel.source.json'),'utf8')).sha256,realModel:true,humanQuality:'not_scored',records},null,2));}
