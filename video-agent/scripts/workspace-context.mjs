...
function runGit(args,{timeout=12000}={}){
  ...
  return {ok:r.status===0,stdout:String(r.stdout||'').trimEnd(),stderr:String(r.stderr||'').trim(),code:r.status};
}
...
function recommendation({isGit,dirty,behind,ahead,statusWarning}){
  if(!isGit)return 'standalone-local: use current files; Git sync is unavailable and must not block the workbench';
  if(statusWarning)return 'preserve-local: Git status coverage is incomplete; inspect statusWarning and never assume a clean checkout';
...
let statusResult=isGit?runGit(['status','--porcelain=v1','--untracked-files=all']):{ok:true,stdout:''};
let statusWarning=null;
if(!statusResult.ok){
  statusWarning='Full Git status failed; submodule worktrees are not verified. '+statusResult.stderr.slice(-1200);
  statusResult=runGit(['status','--porcelain=v1','--untracked-files=all','--ignore-submodules=all']);
  if(!statusResult.ok)statusWarning+=' Parent status also failed: '+statusResult.stderr.slice(-600);
}
...
const dirty=statusResult.ok?dirtyPaths.length>0:null;
const context={
  schemaVersion:1,
  generatedAt:now,
  workspaceRoot:ROOT,
  mode:isGit?'git-checkout':'standalone-local',
  git:{isGit,branch,head,originMain,ahead,behind,dirty,dirtyPaths,artifactDirtyPaths,statusWarning,remote:sanitizeRemote(remoteRaw),fetchAttempted:fetchSoft,fetchWarning},
  ...
  recommendedAction:recommendation({isGit,dirty,behind,ahead,statusWarning}),
