...
public static class NativeSceneJob {
 ...
 [StructLayout(LayoutKind.Sequential)] public struct Accounting { public long UserTime, KernelTime, PeriodUserTime, PeriodKernelTime; public uint PageFaults, TotalProcesses, ActiveProcesses, TerminatedProcesses; }
 ...
 public static uint Active(IntPtr job){int n=Marshal.SizeOf(typeof(Accounting));IntPtr p=Marshal.AllocHGlobal(n);try{if(!QueryInformationJobObject(job,1,p,(uint)n,IntPtr.Zero))throw new System.ComponentModel.Win32Exception(Marshal.GetLastWin32Error());return ((Accounting)Marshal.PtrToStructure(p,typeof(Accounting))).ActiveProcesses;}finally{Marshal.FreeHGlobal(p);}}
...
try {
 ...
 Write-Output ('HF_SCENE_STARTED ' + $jobConfig.runId)
 [Console]::Out.Flush()
 [IO.File]::WriteAllText($jobConfig.gate,'assigned')
 $clock=[Diagnostics.Stopwatch]::StartNew();$timedOut=$false;$cancelled=$false
 while(-not $workerProcess.WaitForExit(50)){
  $cancelled=[IO.File]::Exists($jobConfig.cancelFile)
  $timedOut=$clock.ElapsedMilliseconds -ge [int]$jobConfig.wallMs
  if($cancelled -or $timedOut){[void][NativeSceneJob]::TerminateJobObject($jobHandle,124);$workerProcess.WaitForExit();break}
 }
 ...
 for($attempt=0;$attempt -lt 100 -and [NativeSceneJob]::Active($jobHandle) -gt 0;$attempt++){Start-Sleep -Milliseconds 50}
 $activeProcesses=[NativeSceneJob]::Active($jobHandle)
 $result=[NativeSceneJob]::Read($jobHandle)
 $receipt=[ordered]@{protocolVersion=1;type='windows-job';assigned=$true;activeProcesses=$activeProcesses;pid=$workerProcess.Id;exitCode=$workerProcess.ExitCode;timedOut=$timedOut;cancelled=$cancelled;limitFlags=$result.Basic.Flags;cpuRate=5000;cpuSeconds=$jobConfig.cpuSeconds;processMemoryBytes=$result.ProcessMemory.ToUInt64();jobMemoryBytes=$result.JobMemory.ToUInt64();peakProcessMemoryBytes=$result.PeakProcessMemory.ToUInt64();peakJobMemoryBytes=$result.PeakJobMemory.ToUInt64();stdout=$stdoutTask.Result;stderr=$stderrTask.Result} | ConvertTo-Json -Compress
