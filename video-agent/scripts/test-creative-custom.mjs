...
test('Windows job limits and Chromium sandbox render motion while blocking network, project and file access',async()=>{const result=await runSceneIsolation(await fixture('valid'),config(),{probe:'network'});assert.equal(result.evidence.limitFlags,0x230c);assert.equal(result.evidence.cpuRate,5000);assert(result.evidence.peakJobMemoryBytes>0);assert(result.runtime.motion.every(m=>m.moved));assert(!result.runtime.environmentKeys.some(k=>/CODEX|AUTH|TOKEN|PROXY/i.test(k)));assert(Object.values(result.runtime.boundaryProbe).every(Boolean));});
test('Windows job terminates an unbounded worker and refuses excessive committed memory',async()=>{const timeout=await runSceneIsolation(await fixture('timeout'),config(),{probe:'timeout'});assert.equal(timeout.evidence.timedOut,true);assert.equal(timeout.code,124);const memory=await runSceneIsolation(await fixture('memory'),config(),{probe:'memory'});assert.notEqual(memory.code,0);assert.equal(memory.evidence.timedOut,false);assert.equal(memory.evidence.processMemoryBytes,192*1024**2);});
