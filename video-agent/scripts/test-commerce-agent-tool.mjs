...
await assert.rejects(run({action:'create',request:sample}),/COMMERCE_MATERIALS_BLOCKED/);
const created={ok:true,result:await compileCommerceFixture(sample)};
...
console.log('PASS host-agent rejects unreviewed create; native fixture patch preserves previous revision');
