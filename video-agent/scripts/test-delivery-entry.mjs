...
test('delivery routes fail closed and derive current job and files',async()=>{
 ...
 const call=async url=>{let status,body;const res={setHeader:()=>{},writeHead:(n)=>status=n,end:b=>body=b};await deliveryRoutes(root,{method:'GET'},res,new URL(url,'http://localhost'),{creative,file:async(_req,_res,target)=>{status=200;body=await fs.readFile(target,'utf8');}});return {status,body};};
 ...
 await assert.rejects(call('/delivery-assets/Q02-A/final_video?delivery=formal'),{code:'DELIVERY_NOT_ACCEPTED'});
