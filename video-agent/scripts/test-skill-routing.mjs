...
for(const id of ['product-understanding','marketing-planner','video-director'])assert(caps.some(skill=>skill.id===id),id+' must be registered');
...
assert.equal(caps.find(s=>s.id==='commerce-promo').implementation,'project-tool');
console.log('PASS creation, commerce and editing intents load the registered HyperFrames skill adapters');
