...
const WRITE_TOOLS=new Set(['commerce_create_video','commerce_edit_video','commerce_generate_asset','commerce_job_control','commerce_revision_control','commerce_export']);
...
export function createOpenClawExecutionAuthorizations({file,ttlMs=24*60*60*1000}={}){
 ...
 async function issue({projectId,baseRevisionId,messageId,message,sessionKey,allowedTools=[...WRITE_TOOLS]}){
  ...
  const tools=[...new Set(allowedTools)];if(!tools.length||tools.some(tool=>!WRITE_TOOLS.has(tool)))fail('authorization tool scope is invalid','OPENCLAW_AUTHORIZATION_SCOPE_INVALID',400);
 ...
 async function validateAndBind({tool,input,context}){
  return serialize(async()=>{
   ...
   if(record.sessionKeyHash!==hash(context.sessionKey)||!record.allowedTools.includes(tool))fail('execution authorization context mismatch','OPENCLAW_AUTHORIZATION_SCOPE_INVALID');
