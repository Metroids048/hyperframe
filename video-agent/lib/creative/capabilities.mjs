...
export class CapabilityCatalog {
  ...
  async lockUsedResources(directory){
    ...
    const files=[];for(const r of records.values()){const content=(r.file.startsWith('prompts/commerce/')||localAdapterFiles.has(r.file))?await fs.readFile(path.join(this.root,r.file)):Buffer.from((await this.read(r.file)).content);insist(resourceHash(content)===r.sha256,'已用上下文在打包前变化','RESOURCE_HASH');const relative='resources/'+r.file+(/\.(?:html|js|mjs|py|sh)$/.test(r.file)?'.reference.txt':'');await fs.mkdir(path.dirname(path.join(directory,relative)),{recursive:true});await fs.writeFile(path.join(directory,relative),content);files.push({...r,packagePath:relative});}
