import {formalVideoSnapshot} from './delivery-gate.mjs';
...
export async function deliveryRoutes(root,req,res,url,{file,json,creative}){
 ...
 if(match){
  ...
  let target=asset.target;if(!target){res.writeHead(404);res.end();return true;}
  if(asset.path.endsWith('.mp4')){if(url.searchParams.get('delivery')==='formal')target=await formalVideoSnapshot(root,path.dirname(target),{currentRevisionId:item?.projectId&&creative?.has(item.projectId)?creative.get(item.projectId).currentRevisionId:undefined,getCurrentRevisionId:item?.projectId&&creative?.has(item.projectId)?()=>creative.get(item.projectId).currentRevisionId:undefined});else res.setHeader('X-Delivery-Status','candidate');}
