...
export async function reviewFinalQuality(root,directory,document,{signal,provider}={}){
  ...
  const selected=[...times].sort((a,b)=>a-b),evidence=[];await fs.mkdir(path.join(directory,'final-review'),{recursive:true});
  ...
  for(const [index,t] of selected.slice(0,40).entries()){
