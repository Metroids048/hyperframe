...
export async function reviewFinalQuality(root,directory,document,{signal,provider}={}){
  ...
  const content=[{type:'input_text',text:JSON.stringify({contract:document.businessContract,approvedSourceIndex:admission.assets,binding,creativeDecisionLoop:document.creativeDecisionLoop||null,visualContract:document.visualContract||null,playbackReview:{manifest:playback.clips.map(clip=>({file:clip.file,start:clip.start,end:clip.end})),audioPerceptionVerified:false},observation:'Only these sampled frames from the exported MP4. No audio perception or full continuity coverage. Compare the selected opening with the rejected candidate when evidence is available; do not infer motion or listening from stills.'})}];
  ...
  try{
    ...
    const answer=await provider.structured(guidance.text+'\n只审查本次实际MP4抽帧。从商家、首次观看者、剪辑师视角核对合同。不能推断未观察连续性或听感，不能代签真人。引用提供的frame文件，不自造时间戳。问题按技术缺陷、主体事实、排版美术、叙事节奏、运动衔接、音频或商品表达分类，并给出可执行改法。不要输出未经校准的总分；连续运动和听感保持未验证。',[{role:'user',content}],schema,signal);
