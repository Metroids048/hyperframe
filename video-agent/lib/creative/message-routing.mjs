...
async function legacyRoute(project,message,{provider,signal,document=null,taskMode,taskModeExplicit=false,scenarioId=null,conversation=[],businessIntent=null}={}) {
  ...
  const planAndExecute=/(?:先|给我|设计|制定)[^。！？\n]{0,28}(?:方案|计划)[^。！？\n]{0,28}(?:然后|再|并|并且)[^。！？\n]{0,16}(?:执行|修改|制作|生成)/.test(text);
  if(planAndExecute)return {mode:'plan',quote:message,revisionId:null,assetIds:[],question:'',source:'explicit-plan-and-execute',autoExecute:true};
  ...
  try {
    ...
    // Some OpenAI-compatible gateways normalize punctuation or whitespace in
    // an echoed quote. Repair only equivalent text; never turn invented model
    // wording into user-authored evidence.
    const normalizedQuote=value=>String(value||'').normalize('NFKC').replace(/[\s，。！？、,.!?;；:："“”'‘’「」『』（）()\[\]【】]/gu,'');
    if(modes.includes(result.mode)&&typeof result.quote==='string'&&result.quote&&message.includes(result.quote)===false&&normalizedQuote(result.quote)===normalizedQuote(message)){
