...
const modes=['create','edit','recut','variant','undo','redo','restore','cancel','status','plan','clarify'];
const schema={type:'object',additionalProperties:false,properties:{mode:{type:'string',enum:modes},quote:{type:'string'},revisionId:{type:['string','null']},assetIds:{type:'array',items:{type:'string'}},question:{type:'string'},scenarioId:{type:['string','null'],enum:['product_launch','product_detail','product_demo','product_collection','product_promotion','product_faq','general',null]}},required:['mode','quote','revisionId','assetIds','question','scenarioId']};
export async function routeWorkbenchMessage(project,message,{provider,signal,document=null,taskMode,taskModeExplicit=false,scenarioId=null}={}) {
  ...
  const control={撤销:'undo',撤销上一步:'undo',撤销刚才的修改:'undo',重做:'redo',取消:'cancel',停止:'cancel',查看状态:'status',现在进度:'status',status:'status'}[text.toLowerCase()];
