...
export function animateKeyframe(source,animation){
  ...
  const allowed=new Set(source.objects.map(o=>o.elementId)),invalid=(animation.motionTargets||[]).filter(id=>!allowed.has(id));
  insist(!invalid.length,'动画目标未登记：'+invalid.join(',')+'；本镜头只可使用 '+[...allowed].join(',')+'。保留关键帧，只修改animation，不能动画化未登记的父容器。','CUSTOM_MOTION');
  const ast=parse(animation.timeline,{ecmaVersion:2022});
  for(const statement of ast.body){
    const target=statement.expression?.arguments?.[0]?.value;
    insist(typeof target==='string'&&target.startsWith('#')&&allowed.has(target.slice(1)),'动画选择器未登记：'+String(target)+'；仅可用 '+[...allowed].map(id=>'#'+id).join(',')+'。','CUSTOM_MOTION');
  }
...
export async function inspectKeyframe(document,sceneId,assets,outputDir,runHyperFrames,{signal,atSeconds}={}){
  ...
  const validationFiles=await Promise.all(['lib/creative/custom-source.mjs','lib/creative/isolation.mjs','lib/creative/isolation-protocol.mjs','scripts/native-scene-worker.mjs'].map(async file=>[file,resourceHash(await fs.readFile(new URL('../../'+file,import.meta.url)))]));
  const key=resourceHash({inspectionVersion:3,validationFiles,document:native,assets:assets.map(a=>[a.id,a.sha256]),atSeconds:at}),folder='keyframes/'+sceneId+'-'+key.slice(0,16),directory=path.join(outputDir,folder),file=path.join(directory,'verified.json');
