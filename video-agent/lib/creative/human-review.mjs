...
export function reviewSubmissionError(input,pending){
  if(!['accepted','needs_fix'].includes(input?.status)||typeof input.feedback!=='string'||input.feedback.trim().length<2)return '请填写有效审阅结论和具体反馈';
  if(input.status==='accepted'&&(input.fullVideoObserved!==true||input.businessGoalObserved!==true||pending?.playbackCompleted!==true))return '请播放当前完整文件至结尾、核对业务目标并填写具体反馈';
  return null;
}
...
export async function humanReviewRoute(root,service,req,res,url,{json,jsonBody}){
  ...
  if(req.method==='GET'){
    ...
    const html=`<!doctype html><meta charset="utf-8"><title>当前候选片审阅</title><style>body{max-width:960px;margin:32px auto;font:17px/1.6 system-ui;padding:20px}video{width:100%;max-height:65vh;background:#111}label{display:block;margin:15px 0}textarea{width:95%;height:100px}button{padding:12px}pre{white-space:pre-wrap}</style><h1>候选片审阅</h1><p>本地操作者确认，未核实实名身份。请完整观看当前最终文件并记录具体反馈。</p><p>版本 ${esc(binding.revisionId)} · 文件 ${esc(binding.finalVideoSha256)}</p><h2>原始需求</h2><p>${esc(contract.originalRequest)}</p><h2>自动审查与未覆盖内容</h2><pre>${esc(JSON.stringify({dimensions:report.dimensions,issues:report.issues,unreviewed:report.unreviewed},null,2))}</pre><video id="full-video" controls preload="metadata" src="revisions/${encodeURIComponent(r.id)}/commerce-final.mp4"></video><p id="playback-state">请从头播放到结尾；完成前无法提交完整观看确认。</p><form id="review"><label><input id="full-video-observed" type="checkbox" name="fullVideoObserved" required disabled>我已完整观看这份文件，检查商品、文字、必要动作与衔接</label><label><input type="checkbox" name="audioObserved">我已实际听过这份文件的声音</label><label><input type="checkbox" name="businessGoalObserved" required>我已对照原需求检查商品事实、必要内容和业务目标</label><label>审阅反馈<textarea name="feedback" required minlength="2"></textarea></label><button name="status" value="accepted">认可当前版本</button> <button name="status" value="needs_fix">需要修改</button></form><pre id="result"></pre><script>const reviewToken=${JSON.stringify(token)},video=document.querySelector('#full-video'),observed=document.querySelector('#full-video-observed');video.addEventListener('ended',async()=>{const response=await fetch(location.href,{method:'POST',headers:{'Content-Type':'application/json','X-Review-Token':reviewToken},body:JSON.stringify({action:'playback_completed'})});if(response.ok){observed.disabled=false;document.querySelector('#playback-state').textContent='已播放至结尾。请核对业务目标并填写具体反馈。';}});document.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const response=await fetch(location.href,{method:'POST',headers:{'Content-Type':'application/json','X-Review-Token':reviewToken},body:JSON.stringify({status:e.submitter.value,feedback:f.get('feedback'),fullVideoObserved:f.has('fullVideoObserved'),audioObserved:f.has('audioObserved'),businessGoalObserved:f.has('businessGoalObserved')})});document.querySelector('#result').textContent=JSON.stringify(await response.json(),null,2);};</script>`;
  ...
  if(req.method==='POST'){
    ...
    if(input.action==='playback_completed'){
      if(!pending||pending.bindingHash!==digest(binding)||Date.now()-pending.createdAt>3600000)throw new CreativeError('审阅会话已过期或版本已变化，请重新打开当前版本','HUMAN_REVIEW_CONTEXT',403);
      pending.playbackCompleted=true;json(res,{ok:true,playbackCompleted:true});return true;
    }
    const submissionError=reviewSubmissionError(input,pending);if(submissionError)throw new CreativeError(submissionError,'REVIEW_INCOMPLETE');
