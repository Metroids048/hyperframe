...
export function validateTextStyle(style={}) {
  insist(style&&typeof style==='object'&&!Array.isArray(style)&&Object.keys(style).every(k=>['color','fontSize','fontWeight','offsetY'].includes(k)),'文字样式仅支持颜色、字号和字重','INVALID_TEXT_STYLE');
  ...
  if(style.offsetY!==undefined)insist(Number.isFinite(style.offsetY)&&Math.abs(style.offsetY)<=400,'文字位移必须在400像素内','INVALID_TEXT_STYLE');
  return style;
}
