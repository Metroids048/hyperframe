...
export async function discoverMaterialRoots(root){
 ...
 async function walk(dir,depth=0){if(depth>4)return;const entries=await fs.readdir(dir,{withFileTypes:true}).catch(e=>{if(e.code==='ENOENT')return [];throw e;});const files=entries.filter(e=>e.isFile()&&media.test(e.name)&&!/(?:contact|review-|candidate|poster|crop)/i.test(e.name)).map(e=>path.join(dir,e.name));if(files.length){const real=await fs.realpath(dir),id=createHash('sha256').update(real).digest('hex').slice(0,24);if(!found.some(r=>r.id===id))found.push({id,label:path.relative(root,dir),directory:real,files,videos:files.filter(f=>/\.(mp4|mov|webm)$/i.test(f)).length,images:files.filter(f=>/\.(png|jpe?g|webp)$/i.test(f)).length});}for(const e of entries.filter(e=>e.isDirectory()&&!e.name.startsWith('.')))await walk(path.join(dir,e.name),depth+1);}
