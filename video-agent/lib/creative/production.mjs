import {visualDirectionReference} from './visual-direction-evidence.mjs';
import {inspectionBudget,assertInspectionBudget} from './inspection-budget.mjs';
import {assertNoInternalInputGap} from './production-gaps.mjs';
import {editorialProfile,editorialDiagnostics} from './editorial-strategy.mjs';
...
import {assertCompleteNarration,needsNarrationAlignment,measuredNarrationTranscript} from './narration-timing.mjs';
...
import {durationContract} from './duration-contract.mjs';
import {observationAudioStatus,needsSourceSpeechEvidence,preserveFullOriginalTrack,fullOriginalAudioGraph} from './observation-audio.mjs';
...
import {materialSchema,directionSchema,validateMaterial,validateCreativeDirection,selectStorySources,bindSourceSelectionDocument,canonicalizeSingleAssetReferences} from './commerce-directors.mjs';
...
import {compactStoryEvidenceBatches} from './story-context.mjs';
...
const storyScene=obj(Object.fromEntries(Object.entries({...sceneProperties,paragraphId:str,newInformation:str,resourceId:str,visualDirection:str,layoutVariant:{type:'string',enum:['auto','step-strip','detail-focus']},editorialDecision:obj({placementReason:str,cutBasis:str,subjectProtection:str,audioRelationship:str,transitionPurpose:str}),productionMethod:{type:'string',enum:nativeRecipeContract.methods}}).filter(([key])=>!['effectParamsJson','customSourceJson'].includes(key))));
...
const labels={material:'分析商品与动作证据',product:'建立商品理解',marketing:'制定营销策略',creative:'设计视觉方向',brief:'理解本次要求',observe:'观察真实素材',resources:'选择制作资源',narration:'制作并测量实际旁白',story:'安排整片内容与节奏',director:'生成导演时间线',hyperframes:'映射 HyperFrames 表达',timing:'核对真实声音与动作时间',assemble:'合成原生母工程',quality:'观看实际预览并定位问题'};
...
export async function produceDocument(request,assets,{root,outputDir,signal,provider,runHyperFrames,onStage,onRun,resumeRunId,io={}}={}){
  ...
  let fingerprintInput={request,assets:assets.map(a=>[a.id,a.sha256]),resources:catalog.snapshot?.commit,prompts:await fs.readFile(path.join(root,'prompts/commerce/manifest.json'),'utf8'),pipeline:v3?3:1,scenePackageHash:await scenePackageFingerprint(root,currentContract?.scenarioId),catalogHash:discovery?.data.contentHash||null,implementationHash,model:provider.model||null,reasoningEffort:provider.reasoningEffort||'low'};
  if(resumeRunId){
    // A parser-only contract fix must not invalidate a persisted production
    // checkpoint. Keep the old contract in the fingerprint while executing
    // with the rebuilt, stricter contract above; all media and user text must
    // still match exactly.
    const priorInput=await fs.readFile(path.join(outputDir,'run-input.json'),'utf8').then(JSON.parse).catch(()=>null);
    if(priorInput?.request?.businessContract&&fingerprintInput.request?.businessContract)
      fingerprintInput={...fingerprintInput,request:{...fingerprintInput.request,businessContract:priorInput.request.businessContract}};
  }
  const fingerprint=productionFingerprint(fingerprintInput);
  ...
  // The OpenClaw Responses gateway currently rejects nullable structured
  // fields while normalizing tool arguments (`'text' in null`). Narration
  // revision is optional internal metadata and is not needed for source-only
  // workflows, so keep it out of the wire schema. The executor already treats
  // an omitted field as no revision proposal.
  ...
  async function ask(ctx,stage,data,schema,{images=[],resources=[],extra=''}={}){
    data={...data,editorialProfile:editorialProfile(currentContract?.scenarioId,currentContract?.workflow?.taskMode||currentContract?.taskMode),audioCapabilities:{
      ...
      transcription:{available:typeof provider.transcribe==='function',execution:'provider.transcribe',coverage:'speech timestamps only; not listening or non-speech semantic review'},
      suppliedAudio:assets.filter(a=>a.kind==='audio').map(a=>({id:a.id,name:a.name||a.compiledRef,metadata:a.mediaMetadata,rights:a.rights||null})),
      music:{execution:'schedule supplied audio assets; no implicit music generation or library search'},
      listening:{available:false,coverage:'human listening remains pending; source proxies retain original audio. Missing model listening is a review limitation, not missing source media. Preserve source audio and mark candidate review pending; never invent speech captions.'}
    }};
    if(['R1','R3','CD'].includes(stage))extra+='\n音频能力以audioCapabilities为准；adapter.tools仅列视觉适配工具，不是全部应用能力。已请求旁白且narration.available为true时，后续narration.prepare会调用真实供应商并测量，不因尚无旁白资产要求用户先上传。不能把尚未合成写成已合成，也不能推断供应商一定成功。音乐必须绑定suppliedAudio里的真实音源及已有权利依据，不能假设未挂入工程的音乐已可用。';
    if(['R1','R3','R4'].includes(stage))extra+='\n区分禁止新增叠层与移除源片内容：用户要求保留原片、仅剪辑且不加字幕/标题/图形时，源片已有文字、标识与版式是原片像素，不据此判素材缺失。如果用户明确要求移除源片文字或成片绝对无字，仍须遵守，不能将该要求改成仅禁止新增。media.focusX/focusY控制object-position的0—1对齐比例（0靠左/上，0.5居中，1靠右/下），不是源坐标；横转竖必须结合真实主体位置设置，或在主体无法完整容纳时使用contain。';
    ...
    if(v3)data={...data,scenePackage:sceneContext(scenePackage,stage),productBrief:result(ctx.run,'product')||null,marketingPlan:result(ctx.run,'marketing')||null,creativeDirection:result(ctx.run,'creative')||null,directorTimeline:result(ctx.run,'director')||null,hyperframesDesignPlan:result(ctx.run,'hyperframes')||null};
    if(v3&&stage==='R4')extra+='\nProductBrief、MarketingPlan和CreativeDirection是分镜的上游合同。每幕purpose、newInformation和editorialDecision必须能追溯到营销结构，不得把Hook、卖点证明和CTA退化成通用标题卡。';
    ...
    const contextStage={CD:'R3',MA:'R2',PU:'R2',MP:'R3',VD:'R4'}[stage]||stage;
    const guidance=await catalog.context(contextStage,resources,{phase:data.phase});
  ...
  async function ensureSourceBoundaries(story,ctx){
    if(!v3)return;
    const ranges=story.scenes.flatMap(s=>s.media.filter(m=>byId[m.assetId]?.kind==='video').map(m=>({assetId:m.assetId,startSeconds:m.sourceStartSeconds||0,endSeconds:Math.min(byId[m.assetId].mediaMetadata.duration,(m.sourceStartSeconds||0)+s.durationSeconds*(m.playbackRate||1))})));
  ...
  registry.register('materials.analyze',async(_,ctx)=>{
    ...
    for(let attempt=0;attempt<2;attempt++){
      ...
      const ev=await sourceEvidence(ctx.run,{limit:16,requiredBatchKeys:actionBatches.map(b=>b.key)}),images=selectEvidenceInputs(ev.images,actionBatches.length?12:attempt?8:6).inputs;
      // Keep the complete action sheets on disk, but bound the model payload. A
      // single 4 Hz inspection can contain hundreds of repeated frame records;
      // sending all of them together with base64 images can exhaust the service
      // before the provider has a chance to return a structured result.
      const boundedActionInspections=actionBatches.map(batch=>{
  ...
  registry.register('product.understand',async(_,ctx)=>{
    const evidence=await sourceEvidence(ctx.run,{limit:16});
    const product=await ask(ctx,'PU',{message:request.message,requestFacts:result(ctx.run,'brief').request.facts,brief:result(ctx.run,'brief'),observations:result(ctx.run,'observe'),material:result(ctx.run,'material'),brand:request.brand||request.product?.brand||null,platform:request.platform||currentContract?.platform||null,assets:assets.map(a=>({id:a.id,kind:a.kind,metadata:a.mediaMetadata})),evidenceIndex:evidence.selection},productBriefSchema,{images:selectEvidenceInputs(evidence.images,16).inputs,extra:'融合用户文本、图片观察和视频时间证据，输出唯一 ProductBrief v2，不写分镜。product_name未知时写“未确认商品名称”，category必须使用能由素材支持的中性类别。visual_features只写可见特征；selling_points可以来自用户原话或可见使用价值，但每条必须有证据引用，confidence为0到1。证据ref只能使用request.fact.N、material.fact.N、observation.<assetId>或asset:<assetId>@<start>-<end>；reason说明它支持什么，不能从文件名推断。target_customer和usage_scenarios是营销建议，不升级为产品事实。brand_style只使用已提供品牌信息或从素材得到的保守视觉描述。forbidden_claims必须合并用户禁用项、material.unsupportedClaims和无法证实的性能、价格或促销。'});
    validateProductBrief(product,assets);return saveJSON('product-brief.json',product);
  });
  ...
  registry.register('creative.direct',async(_,ctx)=>{
    const evidence=await sourceEvidence(ctx.run,{limit:8}),reference=await visualDirectionReference(root,outputDir,{signal,message:request.message,scenarioId:currentContract?.scenarioId,category:result(ctx.run,'product')?.category,marketingObjective:result(ctx.run,'marketing')?.marketing_objective,creativeDirection:result(ctx.run,'creative')?.visualDirection});
    const direction=await ask(ctx,'CD',{message:request.message,brief:result(ctx.run,'brief'),productBrief:result(ctx.run,'product'),marketingPlan:result(ctx.run,'marketing'),material:result(ctx.run,'material'),templates:scenePackage.templates,visualEvidence:evidence.selection,qualityReference:reference.receipt},directionSchema,{images:[...selectEvidenceInputs(evidence.images,8).inputs,...reference.inputs],extra:'营销目标、受众、Hook、故事顺序和CTA以MarketingPlan为上游合同；本步骤只把它转成视觉、节奏、声音、Hero和结尾方向。只从给定业务模板中选择ID；这是结构，不是固定MP4。风格必须服从素材，演示动作高于动效；无证据不写卖点。根据本次真实图片决定主体占比、明暗层级和文字位置，不只读素材文字摘要。参考画面仅比较信息层级、主体识别和完成度；不得复制参考片的配色、固定构图或商品事实。若需要裁切或局部放大，必须说明已观察的保护区域与稳定窗口；不能为填满画面裁掉动作。不生成最终工程。'});
    validateCreativeDirection(direction,{templates:scenePackage.templates.businessTemplates,marketingPlan:result(ctx.run,'marketing'),scenarioId:currentContract?.scenarioId});
    await saveJSON('creative-direction-acceptance.json',{status:'accepted_for_production',acceptedBy:'creative-direction-contract-validator',directionHash:resourceHash(direction),productBriefHash:resourceHash(result(ctx.run,'product')),marketingPlanHash:resourceHash(result(ctx.run,'marketing')),checks:['required direction fields','known business template','usable visual functions','scenario action protection','explicit do-not list'],humanQuality:'not_assessed'});
    return saveJSON('creative-direction.json',direction);
  });
  registry.register('resources.plan',async(_,ctx)=>{
    ...
    if(v3){const planner=new HyperFramesResourcePlanner(discovery,catalog.executionCandidates({visualInputCount,message:request.message+' '+result(ctx.run,'brief').capabilities.join(' '),assets}));const plan=planner.plan({message:request.message,...scenePackage.resourceProfile,sourceKind:[...new Set(assets.filter(a=>['image','video'].includes(a.kind)).map(a=>a.kind))].join('|')||'text-only',visualFunctions:result(ctx.run,'creative')?.visualFunctions||[],businessPurpose:result(ctx.run,'creative')?.singleSentenceIdea,visualPurpose:result(ctx.run,'creative')?.visualDirection,output:request.output,mediaCount:visualInputCount,mediaKinds:assets.map(a=>a.kind),actionProtected:requiresActionProtection(currentContract)});await saveJSON('hyperframes-discovery.json',plan);ctx.run.artifacts.resourceDiscovery=plan;candidates=candidates.map(c=>({...c,eligible:c.eligible&&plan.adapterChecks.find(a=>a.id===c.id)?.eligible}));}
  ...
  registry.register('narration.prepare',async(input,ctx)=>{
    ...
    const profileConfig=JSON.parse(await fs.readFile(path.join(root,'config/voice_profiles.json'),'utf8'));
    const configuredProfiles=[...(profileConfig.profiles||[]),...(profileConfig.local_profiles||[])];
    const profileCatalog=await buildVoiceProfiles(voiceCatalog||{engine:'fallback',voices:[{id:'zf_001'},{id:'zm_009'}]},{directory:outputDir,baseProfiles:configuredProfiles});
    const requirement=audioRequirement({message:request.message,marketingPlan:result(ctx.run,'marketing'),sceneId:currentContract?.scenarioId});
    const candidates=voiceCandidates(profileCatalog.profiles,requirement,{limit:12});
    await saveJSON('audio-requirement.json',requirement);await saveJSON('voice-candidates.json',{requirement,candidates});
    const catalogIds=voiceCatalog?.voices?.map(v=>v.id)||['zf_001','zm_009'];
    const retainedVoice=savedScript?.value?.voice;
    const allowedVoices=[...new Set([...candidates.map(v=>v.voice_id),...(retainedVoice&&catalogIds.includes(retainedVoice)?[retainedVoice]:[])])];
    ...
    const script=reusableScript||await ask(ctx,'R4',{phase:'narration-script-before-measured-timing',audioRequirement:requirement,voiceCandidates:candidates,retainedScript:savedScript?.value?.text||null,message:request.message,brief,productBrief:result(ctx.run,'product'),marketingPlan:result(ctx.run,'marketing'),observations:result(ctx.run,'observe'),targetSeconds:brief.request.output.durationSeconds},obj({text:str,voice:{type:'string',enum:allowedVoices},basis:str}),{extra:'这是Voice Matching Pipeline的LLM rerank阶段，只能从voiceCandidates选择真实voice_id，并在basis说明与AudioRequirement的匹配理由；不得接触完整300+目录。retainedScript非空时逐字保留，只重新选择候选中的可用音色。仅在用户要求旁白时撰写完整中文配音稿。用户给定逐字台词时必须保留，不添加未证实参数、价格、性能、配件或操作。其余依据ProductBrief和素材证据写中性讲解；不把证据缺口念成制作日志。保守按每秒2.5—3个汉字安排，留出操作与阅读时间，不故意加停顿凑满时长。实际时长由下一步合成测量，再由导演编排画面。'});
    ...
    const providerTranscript=provider.lastSpeechTranscript,alignedLocally=needsNarrationAlignment(providerTranscript);
    if(providerTranscript)await saveJSON('provider-transcript-'+id+'.json',providerTranscript);
    const alignment=await measuredNarrationTranscript(provider,file,providerTranscript,signal),transcript=alignment.transcript;await saveJSON('narration-alignment.json',{method:alignment.method,granularity:alignment.granularity,limitation:alignment.limitation,sourceSha256:asset.sha256});
    asset.audioRole='narration';asset.speechRequest={text:script.text,voice:provider.lastSpeechMetrics?.voice||script.voice,rate:1};asset.providerTranscript={...transcript,sourceSha256:asset.sha256,source:alignment.method};
    if(revision){
  ...
  async function inspectWithBudget(ranges,ctx,action){
    ...
    try{
      assertInspectionBudget(inspections,ranges,{action});
  ...
  registry.register('story.plan',async(_,ctx)=>{
    ...
    const timingContract=durationContract(request.message,brief.request.output.durationSeconds);
    let story,lastError=ctx.run.artifacts.storyValidationError;for(let attempt=0;attempt<3;attempt++){
    const fallbackModeNote='';
    // Reduced from 24 to 8 to avoid OPENCLAW_STAGE_TIMEOUT on heavy story planning stages.
    // Prioritize: 1 contact sheet + key product moments + critical action frames.
    const evidence=await sourceEvidence(ctx.run,{limit:8,requiredBatchKeys:requiresActionProtection(currentContract)?(ctx.run.artifacts.actionInspections||[]).map(b=>b.key):[]});
    const imageSelection=selectEvidenceInputs(evidence.images,8);
    story=canonicalizeSingleAssetReferences(await ask(ctx,'R4',{timingContract,sourceEvidence:evidence.selection,message:request.message,brief,attempt,validationError:lastError?.message,priorStory:story||result(ctx.run,'story')||null,additionalSourceEvidence:compactStoryEvidenceBatches([...(ctx.run.artifacts.storyInspections||[]),...(ctx.run.artifacts.actionInspections||[])]),evidenceImageBudget:{sent:imageSelection.sent,available:imageSelection.available,omitted:imageSelection.omitted,reason:'model context budget; omitted images were not sent'},availableTools:[{name:'assets.inspect_actions',...inspectionBudget(ctx.run.artifacts.actionInspections||[],{action:true}),sampleFps:4,playableProxy:true},{name:'assets.inspect_ranges',...inspectionBudget(ctx.run.artifacts.storyInspections||[])}],materialAnalysis:result(ctx.run,'material')||null,observations:result(ctx.run,'observe'),narration:{...result(ctx.run,'narration'),revisionPolicy:narrationRevisionPolicy(request,result(ctx.run,'narration'),ctx.run.artifacts.narrationHistory||[])},transcripts:await readJSON('transcripts.json'),assets:assets.map(a=>({id:a.id,kind:a.kind,metadata:a.mediaMetadata})),resources,nativeRecipeContract,fontContract},withKnownFacts(runtimeStorySchema,brief.request.facts),{images:imageSelection.inputs,resources:[],extra:fallbackModeNote+'只输出整片故事和精确镜头时间，不写HTML。音频排程必须写进audio数组：每项assetId、volume、sourceStartSeconds、startSeconds（成片起点）、durationSeconds（播放长度）；连续整段可将后两项填null。分段旁白为同一assetId写多项，源时间必须按顺序完整覆盖、无重复无遗漏，成片段可留阅读停顿但不能重叠或越界；原速播放且边界避开实测词内。只在summary写分段计划不会执行，禁止如此冒充排程。需要确认短动作起止时用inspectActions请求密集观察：每次至多3段，每段12秒，工具生成4Hz逐帧时间标记接触表和可播放源片段。4Hz证据只支持约0.25秒粒度的判断，不能宣称模型完整播放或帧级精确核验。观察宽区间用inspectRanges，两种工具分开记录预算。没有请求时inspectActions为空。若离散观察不足，先通过inspectRanges请求检查已有源片的具体区间（每次最多3段、每段30秒），本次scenes可留空，下一轮会得到真实工具图片及秒数。不要把未观察当成不可获得素材；也不把候选窗口当全部可用素材。已得到的补充观察不要重复请求。确有不可取得的必需资料时填写blockingGaps并解释缺口；已观察到的必要动作原速无法容纳于指定时长时，也用blockingGaps给出最小用时及可调整的时长，不能把时长冲突说成素材缺失。无法给出分镜时必须明确填写观察请求或blockingGaps，不得将scenes和这些字段同时留空。不能悄悄缩时长、补写事实或空转。有效完整分镜时两个数组均为空。按内容决定镜头数，不平均分配。每个镜头必须带新的信息或明确观看作用，paragraphId引用段落。editorialDecision分别说明为何放在此处、实测源画面或声音支持的入出点、主体保护、与原声或旁白的关系、转场表达目的；无听音能力时明确未试听，不虚构声音衔接判断。硬切也是有效选择，transitionPurpose说明即可。不要把几个相似状态各自长时间停留当新信息；对连续超过8秒的镜头说明期间实际发生什么变化、为什么值得观看。durationSeconds明确到1/30秒；遵守timingContract：exact时总帧数严格等于targetFrames；approximate时允许minFrames到maxFrames，以真实素材自然结束为优先，并在summary说明实际总长与原因，不能重复、定格或改变速度补齐近似目标。cut时总帧数为各幕之和，其他转场每处重叠0.3秒。sourceEvidence.sourceBounds的视频终点来自实际工作媒体ffprobe，duration是包含音轨的容器长度；视频终点之后的纯音轨尾部不是缺少画面观察，不再请求不存在的视频帧。完整原声仍保留全长，视觉覆盖校验仅到已探测的视频终点，不冒充连续观片。容器、视频、音轨时长在一帧内的差别属于帧量化范围，不是必须向用户补料的缺口；源尾取覆盖实际媒体终点的输出帧边界，源区间仍由执行器验证，不能自称已完整观片。视频源区间必须足够且不重复凑时长，不改变播放速度除非用户明确要求。效应可为custom-native，后续镜头制作者处理。至少让有需要的关键镜头获得原创设计，但不要强制每幕动效或改掉自然实拍剪辑。按每镜头表达任务从resources.candidates中eligible且compatible的候选选择resourceId，selected仅为整片建议，不限制镜头；没有合适资源用native-original。productionMethod为footage-cut（单段实拍且无文字）、parameterized（符合nativeRecipeContract的常规布局）、composition-adapt（组合或局部适配）、original（必要原创）。常规镜头优先复用，关键创意不强制降级。layoutVariant选择auto、step-strip或detail-focus：step-strip仅用于video-text-pivot的一路视频及一至两项简短步骤文字，媒体与步骤区完全分开；detail-focus仅用于comparison-split的两路已观察媒体，以大整体和次级局部联动。其他资源填auto，不能为了套版式而删除已有内容或伪造第二路素材。visualDirection说明商品与文字主次、开场吸引点、此镜头新增理解及需要克制的地方。没有价格就没有价格段；用户要求开头价格则放开头。每镜头最多4媒体、32文字，整片最多300原生节点是执行预算。模板仅提供结构和视觉语言，素材、文字、顺序、镜头数及时间必须服从本次输入；相同素材换目标应改变有意义的内容取舍。先观察素材再安排镜头，自拟动作无素材支持时修改自己的计划，仅用户明确必需内容缺失时才输出blockingGaps。comparison-split必须绑定至少两路真实媒体窗口，可为同一商品的不同已观察源区间，不暗示竞品对比；只有一路媒体时选择低遮挡标注或原生布局；grid-card-assemble条目数量由已提供内容决定；video-text-pivot须保留动作可见和视频持续播放，不以大色块覆盖。保持原声时audio列出有声视频assetId/volume:1/sourceStartSeconds:0；原声实际随每个源镜头裁切。如果narration.enabled，使用其真实asset.id音轨和transcript实测时间。如果revisionPolicy.allowed且Agent自拟旁白比已观察的对应画面长，使用narrationRevision提交缩短的完整稿和具体时长冲突依据，由工具重新合成测量；这属于内部计划修复，不是需要用户补拍的缺口。修订必须保留事实、音色、业务目标和用户总片长；不能改用户逐字稿或已确认声音。无此冲突时narrationRevision为null，并保留当前完整声音。只使用brief.request的已确认事实，事实ID按fact-1顺序。设计颜色均#RRGGBB。typeScale给当前输出尺寸的像素字号，safeMarginPx给安全边距；labelStyle统一全片标签形状、边距、线条与层级，镜头制作必须继承，不各自发明字体和标签风格。'}),assets);
    ...
    try{validateRequestedTransitionPlan(story,request.message);validateStory(story,brief,resources);await ensureSourceBoundaries(story,ctx);if(v3){const selected=selectStorySources(story,assets,result(ctx.run,'material'),{demo:requiresActionProtection(currentContract),evidenceIndex:await readJSON('source-evidence-index.json')});await saveJSON('source-selections.json',selected);await saveJSON('hyperframes-resources.json',story.scenes.map((shot,i)=>({sceneIndex:i,...new HyperFramesResourcePlanner(discovery,resources.candidates).plan({...scenePackage.resourceProfile,businessPurpose:shot.newInformation,visualPurpose:shot.visualDirection,sourceKind:shot.media.map(m=>byId[m.assetId]?.kind).join(' ')}),chosenResourceId:shot.resourceId})));const visualContract=buildVisualContract({creativeDirection:result(ctx.run,'creative'),story,output:brief.request.output,fontContract,source:'story.plan'});validateVisualContract(visualContract,{allowedFontFamilies});await saveJSON('visual-contract.json',visualContract);}documentFromModelPlan(request,assets,nativePlan(ctx.run,{},story));break;}catch(error){lastError=error;await saveJSON('failed-story-'+attempt+'.json',{story,error:{code:error.code,message:error.message}});if(repairRoute(error)!=='scene'||attempt===2)throw error;}
    }
    await saveJSON('editorial-diagnostics.json',editorialDiagnostics(story));ctx.run.plan={paragraphs:story.paragraphs,sceneCount:story.scenes.length};return saveJSON('story-plan.json',story);
  });
  ...
  registry.register('hyperframes.adapt',async(_,ctx)=>{
    const plan=buildHyperFramesDesignPlan({directorTimeline:result(ctx.run,'director'),story:result(ctx.run,'story'),creativeDirection:result(ctx.run,'creative'),resourcePlan:result(ctx.run,'resources'),output:result(ctx.run,'brief').request.output,scenarioId:currentContract?.scenarioId});
    return saveJSON('hyperframes-design-plan.json',plan);
  });
  ...
  async function buildShotInSlot(ctx,index,feedback){
    const story=result(ctx.run,'story'),shot=story.scenes[index],timing=result(ctx.run,'timing').scenes[index],sceneId=timing.id,directorShot=result(ctx.run,'director')?.shots[index],hfBinding=v3?directorBinding(result(ctx.run,'hyperframes'),sceneId):null;
    ...
    const staticShotSchema=obj({...shotSchema.properties,keyframeAtSeconds:{type:'number',enum:keyframeChoices},mediaBindingGap:str});
    ...
    const adjacentKeyframes=await Promise.all([index-1,index+1].filter(i=>i>=0&&i<story.scenes.length).map(async i=>{const record=await readJSON('keyframe-'+i+'.json').catch(()=>null);return record?{sceneId:story.scenes[i].id,evidence:record.evidence?.folder&&record.evidence?.image?record.evidence.folder+'/'+record.evidence.image:null}:null;}));
    const packet={message:request.message,sceneId,sceneStartSeconds:timing.startFrame/FPS,shot,directorShot,hyperframesBinding:hfBinding,visualContract:await readJSON('visual-contract.json').catch(()=>null),productBrief:result(ctx.run,'product'),marketingPlan:result(ctx.run,'marketing'),nativeObjectRefs:{media:shot.media.map((m,i)=>({ref:'media-'+(i+1),assetId:m.assetId,kind:byId[m.assetId].kind,elementTag:byId[m.assetId].kind==='image'?'img':'div'})),text:shot.text.map((t,i)=>({ref:'text-'+(i+1),role:t.role,text:t.text})),rule:'objects只能引用此列表或已声明的装饰shape。源片自带文字/品牌仍在视频里，不是额外原生文字，不得重复制作或增加text-N。'},output:result(ctx.run,'brief').request.output,design:story.design,fontContract,keyframeChoices,previous:story.scenes[index-1]||null,next:story.scenes[index+1]||null,adjacentKeyframes,observations:result(ctx.run,'observe').observations.filter(o=>shot.media.some(m=>m.assetId===o.assetId)),feedback:feedback||null,repairInstruction:feedback?'在现有源码中做解决已报告问题所需的最小修改；保持所有已有原生对象映射，不重新设计本镜头。':null};
    ...
    if(!feedback){
      ...
      for(let attempt=0;!keyframe&&attempt<3;attempt++){
        const answer=await ask(ctx,'R5',{...packet,phase:'static-keyframe',attempt,error:lastError?.message,previousSource:lastSource||packet.previousSource},staticShotSchema,{images,resources:shot.resourceId==='native-original'?[]:[shot.resourceId],extra:CUSTOM_SOURCE_CONTRACT.replaceAll('customSourceJson','source')+'\n这里只制作最清晰的静态关键画面，尚未制作动画。mediaBindingGap通常为空；如果既定设计必须有多个同源视频窗口但nativeObjectRefs未声明足够媒体，填写具体缺口交回分镜，不能自行复制视频或返回缺窗口的画面冒充完成。根据已提供的真实候选帧，从keyframeChoices选择主体与动作最清楚的本镜头秒数，返回keyframeAtSeconds；不要固定使用中点。静态图没有可见时序差异时也从此列表选择。timeline必须为空字符串，motionTargets必须为空数组，全部获准文字在CSS静态状态清晰可见，不能使用opacity:0隐藏。保留真实实拍主体与所有获准原生对象。不要为动效预置不可见状态。应用会先运行真实布局/对比度检查并截图评审，再单独请求动画。'});
        try{
          if(answer.mediaBindingGap?.trim())throw Object.assign(Error(answer.mediaBindingGap),{code:'KEYFRAME_MEDIA_BINDING',issues:[{severity:'major',repairKind:'media-binding',problem:answer.mediaBindingGap,repair:'由分镜为需要的同源同步多窗口显式增加媒体绑定，保持原始媒体、源时间、播放率、文字、时长和音轨策略。'}]});
          ...
          const review=await ask(ctx,'R6',{phase:'static-keyframe',captionStage:{requested:needsSpeechCaptions(result(ctx.run,'brief')),execution:'project.assemble after all scenes',presentInThisFrame:false,verification:'full composition review remains required'},sceneId,shot,design:story.design,output:native.output,keyframeAtSeconds:answer.keyframeAtSeconds,addedTextObjects:native.nodes.filter(n=>n.sceneId===sceneId&&n.kind==='text').map(n=>({id:n.id,text:n.params.text})),evidence:inspected.folder+'/'+inspected.image},obj({issues:list(obj({severity:{type:'string',enum:['major','minor']},repairKind:{type:'string',enum:['layout','source-selection','text-evidence','text-timing','fact-binding','text-contract']},problem:str,repair:str})),summary:str}),{images:[...sourceBoundaryImages,...matchingSourceImages,...keyframeImage],extra:'此处是单镜头静态检查；captionStage.requested=true时，旁白转写字幕将在project.assemble调用真实字幕执行器加入，此帧尚无字幕不构成镜头缺陷，也不得要求镜头作者复制整段旁白。最终合成必须继续核查真实字幕。商品标题、必要免责声明等仍须实际声明并显示，不能用尚未实现的全局层搪塞。对照同一时刻未包装源画面和合成关键帧，以及addedTextObjects明确列出的应用文字，区分源实拍自带的文字/标识与应用新增叠层；只有未包装源画面中也存在的字样才能称为源片烧录文字。源片已有标识不是应用虚构文案，不能仅因模型自己写的visualDirection不强调品牌就要求移除、遮盖或裁掉它。原用户要求高于模型创意说明。确实遮挡必需动作时归source-selection，选择真实可用源区间，不让布局作者以色块覆盖商品或删除原片标识。另提供源选段首末帧，用来核对入点和末帧的可见动作与本镜头说明是否相符；不能仅凭离散帧宣称完整连续动作通过。源边界未加包装，不检查其文字布局。检查静态关键画面中商品、文字主次、主体裁切和中文可读性。只报告图片可确认的问题；尚未添加动画，不把静止当缺陷。major需具体局部修复。repairKind仅在源区间或动作确实错误时为source-selection；对比度、遮挡、排版为layout；事实冲突为fact-binding。源片烧录标题与新增文字重复、或必须删除新增文字才能解决时归text-contract，由故事导演修订文字对象合同；layout只可调整样式，不能要求镜头作者删除、隐藏合同内对象。'});
  ...
  async function replanSourceShot(ctx,index,feedback){
    ...
    const removeRedundantText=feedback.some(changesTextContract),allowDuplicateMedia=feedback.some(i=>i.repairKind==='media-binding'),allowRequiredText=feedback.some(i=>i.repairKind==='text-contract');
    let repaired,rejectedRepair=null;
    for(let repairAttempt=0;repairAttempt<2;repairAttempt++){
    repaired=await ask(ctx,'R4',{allowRequiredText,allowDuplicateMedia,repairAttempt,rejectedRepair,sourceChangeRequired:feedback.some(i=>i.repairKind==='source-selection'),removeRedundantText,sourceEvidence:evidence.selection,phase:'repair-one-source-selection',index,story:original,brief,observations:result(ctx.run,'observe'),additionalSourceEvidence:compactStoryEvidenceBatches([...(ctx.run.artifacts.storyInspections||[]),...(ctx.run.artifacts.actionInspections||[])]),sourceMetadata:assets.map(a=>({id:a.id,...a.mediaMetadata})),feedback},withKnownFacts(storyScene,brief.request.facts),{images:evidence.images,extra:(allowRequiredText?'若审查指出缺少用户明确要求且尚未声明的文字对象，允许在现有text末尾追加逐字来自用户原话的说明，factRefs为空且role不是price/cta；保留原有文字及顺序。不得只在设计说明中承诺一个未实现的全局层。这个例外优先于文字数量不变；旁白字幕由合成阶段生成，不在镜头重复添加全文。\n':'')+(allowDuplicateMedia?'本次修复媒体绑定：允许追加既有media条目的精确副本以支持同源同步局部窗口，最多4路；原条目顺序和属性保持，不能改变源时间、播放率、素材身份、时长、文字、声音策略。这个例外优先于媒体数量不变；其他修复不允许增加媒体。\n':'')+(removeRedundantText?'本次已由真实关键画面审查确认文字合同冲突。允许删除没有factRefs、不是price/cta且不属于用户原话的重复新增文字，保留其余文字顺序和内容，不新增文字；也可保留文字改选无重复烧录文字的已观察源片。这个例外优先于下述文字数量不变要求。\n':'')+'画面检查指出源片选段、重复文字或模型自拟观察说明与画面证据不符。没有用户事实引用的自拟分类、性能暗示也需改为可见的中性描述，不向用户索取Agent自行补写的主张。仅修复这个镜头：若画面正确但模型自拟的中性说明不符，可根据真实证据纠正该说明，不必换掉正确实拍。用户消息中逐字提供的文字、有factRefs的文字、价格与CTA必须原样保留。文字数量、角色、事实引用、媒体数量、时长、段落和资源ID不变；更正说明不得新增数值、参数、价格或无法从实际画面确认的商品事实。不改其他镜头或声音策略。不要重复其他镜头补时长，不编造缺失动作或商品关联。片尾不能在新动作中间戛然而止；依据实际接触表选择能自然结束的真实区间，不能冻结、慢放或循环。返回完整单镜头；sourceStartSeconds实际影响原生视频及同步原声音轨。'});
    validateShotRepair(original.scenes[index],repaired,request.message,{removeRedundantText,allowDuplicateMedia,allowRequiredText});
    try{requireSourceChange(original.scenes[index],repaired,feedback);break;}catch(error){
      await saveJSON('source-repair-rejected-'+index+'-'+Date.now()+'.json',{original:original.scenes[index],repaired,feedback,error:{code:error.code,message:error.message}});
      if(error.code!=='REPLAN_NO_PROGRESS'||repairAttempt===1)throw error;
      rejectedRepair={scene:repaired,error:error.message,requiredAction:'源取景问题必须改变实际sourceStartSeconds或合法素材，文字说明和CSS调整不改变源区间；依据已观察证据选取，不能编造可用时码。'};
    }
    }
    ...
    if(allowDuplicateMedia)insist(resourceHash(native.audioGraph)===resourceHash(documentFromModelPlan(request,assets,nativePlan(ctx.run,{},original)).audioGraph),'同源窗口修复不能重复原声或改变音轨','REPLAN_SCOPE');
  ...
  registry.register('scene.author',async({index},ctx)=>{
    ...
    for(;;){try{return await buildShot(ctx,index);}catch(error){
      if(!['source-selection','fact-binding','media-binding'].includes(repairRoute(error))||signal?.aborted)throw error;
  ...
  async function assemble(ctx){for(const named of ctx.run.artifacts.resourceDiscovery?.requests||[])if(!named.negated&&named.canonicalId!=='chromatic-radial-split')insist(result(ctx.run,'story').scenes.some(s=>s.resourceId===named.canonicalId),'分镜未执行指定资源：'+named.canonicalId,'RESOURCE_UNAVAILABLE');const sources={},assemblyReceipts={};for(const [i,shot] of result(ctx.run,'story').scenes.entries()){const record=await readJSON(result(ctx.run,'shot-'+i).file);insist(resourceHash(record.source)===result(ctx.run,'shot-'+i).sourceHash,'镜头检查点内容变更','SCENE_HASH');const kinds=shot.media.map(m=>byId[m.assetId].kind);sources[i]=normalizeShotSource(record,kinds);
    ...
    const document=documentFromModelPlan(request,assets,nativePlan(ctx.run,sources));if(currentContract)document.businessContract=currentContract;document.storyPlan={paragraphs:result(ctx.run,'story').paragraphs,scenes:result(ctx.run,'story').scenes.map((s,i)=>({sceneId:document.scenes[i].id,paragraphId:s.paragraphId,newInformation:s.newInformation,visualDirection:s.visualDirection,layoutVariant:s.layoutVariant||'auto',editorialDecision:s.editorialDecision||null}))};document.editorialReview=editorialDiagnostics(result(ctx.run,'story'),sources);document.editorialReview.sourceRevisionId=document.revisionId;document.editorialProfile=editorialProfile(currentContract?.scenarioId,currentContract?.workflow?.taskMode||currentContract?.taskMode);document.timingPlan=result(ctx.run,'timing');document.production={runId:ctx.run.id,inputFingerprint:fingerprint,workflowVersion:v3?3:1,stages:v3?['R0','R1','R2','R3','R4','R5','R6','R7','R8','R9','R10','R11','R12']:null,scenePackage:scenePackage?.hash||null,catalog:discovery?.data.contentHash||null};document.dependencyLock={...document.dependencyLock,resources:catalog.snapshot?.commit,prompts:resourceHash(await fs.readFile(path.join(root,'prompts/commerce/manifest.json'),'utf8'))};document.resourceReceipts=Object.keys(sources).map(i=>assemblyReceipts[i]);
    if(v3){document.productBrief=result(ctx.run,'product');document.marketingPlan=result(ctx.run,'marketing');document.directorTimeline=result(ctx.run,'director');document.hyperframesDesignPlan=result(ctx.run,'hyperframes');document.visualContract=await readJSON('visual-contract.json').catch(()=>null);document.creativeDecisionLoop=await readJSON('opening-candidates.json').catch(()=>null);document.production.workflowVersion=4;document.production.stages=['R0','R1','R2','PU','MP','CD','R3','R4','VD','HF','R5','R6','R7','R8','R9','R10','R11','R12'];}
    ...
    const hfPlans=await readJSON('hyperframes-resources.json').catch(()=>[]),storyScenes=document.storyPlan?.scenes||[];
    const shotBindings=document.resourceReceipts.map(receipt=>{
      const scene=storyScenes.find(item=>item.sceneId===receipt.sceneId),plan=hfPlans.find(item=>item.chosenResourceId===receipt.resourceId)||null;
      const candidate=plan?.selected?.find(item=>(item.canonicalId||item.id)===receipt.resourceId)||null;
      const check=plan?.adapterChecks?.find(item=>item.id===receipt.resourceId)||null;
      const matchedFunctions=candidate?.functionalMatch?.matches||[];
      const selectionReason=candidate?`依据镜头目的“${scene?.newInformation||scene?.purpose||'未登记'}”选择；匹配视觉功能：${matchedFunctions.join('、')||'无功能命中，需按镜头说明核对'}；适配器${check?.eligible?'兼容':'状态待核验'}`:`镜头目的“${scene?.newInformation||scene?.purpose||'未登记'}”不需要额外注册资源，使用原生 HyperFrames 布局`;
      return {shotId:receipt.sceneId,purpose:scene?.newInformation||null,businessPurpose:scene?.purpose||scene?.visualDirection||null,resource:receipt.resourceId,method:receipt.method||receipt.execution||null,compatibility:receipt.compatibility||check||null,selectionReason,editorialDecision:scene?.editorialDecision||null,discoveryReason:plan?.whySelected||null,adapterReasons:check?.reasons||[],sourceFiles:candidate?.sourceFiles||[],runtime:'0.8.33'};
    });
    await saveJSON('hyperframes-resource-receipt.json',{
      ...
      selectedResources:shotBindings.map(({resourceId,...binding})=>({resourceId:binding.resource,sceneId:binding.shotId,method:binding.method,compatibility:binding.compatibility,selectionReason:binding.selectionReason,sourceFiles:binding.sourceFiles,runtime:binding.runtime})),
      reason:orchestration.resourceReceipt?.reason||shotBindings.map(binding=>`${binding.shotId}: ${binding.selectionReason}`).join('；'),
      shotBindings,
    });
  ...
  const phaseTools=[['brief','brief.parse'],['observe','assets.observe'],...(v3?[['material','materials.analyze'],['product','product.understand'],['marketing','marketing.plan'],['creative','creative.direct']]:[]),['resources','resources.plan'],['narration','narration.prepare'],['story','story.plan'],...(v3?[['director','video.direct'],['hyperframes','hyperframes.adapt']]:[]),['timing','timing.verify']];
  const kernel=new AgentKernel({registry,store,maxSteps:1200,maxModelCalls:128,onProgress:async run=>{await saveJSON('production-run.json',run);await onRun?.(run);},planner:async({run})=>{
    ...
    if(result(run,'story')?.blockingGaps?.length){assertNoInternalInputGap(result(run,'story').blockingGaps);return {kind:'need_user',gaps:result(run,'story').blockingGaps};}
    if(result(run,'resources')?.blockingGaps?.length){assertNoInternalInputGap(result(run,'resources').blockingGaps);return {kind:'need_user',gaps:result(run,'resources').blockingGaps};}
    ...
    // A real preview is a creative decision point. In the v3 commerce path
    // the director compares two materially different openings before the
  ...
  try{
    if(resumeRunId){
      ...
      const artifacts={material:'material-analysis.json',product:'product-brief.json',marketing:'marketing-plan.json',creative:'creative-direction.json',brief:'brief-plan.json',observe:'observations.json',resources:'resource-plan.json',story:'story-plan.json',director:'director-timeline.json',hyperframes:'hyperframes-design-plan.json',timing:'timing-plan.json',narration:'narration.json',quality:'quality-report.json','opening-candidates':'opening-candidates.json','director-replan':'director-replan.json'};
    ...
    const finalDocument=await readJSON('document.json');if(v3){scenePackage??=await loadScenePackage(root,result(run,'brief')?.productionBinding?.contract?.scenarioId);insist(scenePackage,'完成工程缺少有效场景规则包','SCENE_PACKAGE');finalDocument.scenePackage={id:scenePackage.id,version:scenePackage.version,hash:scenePackage.hash};finalDocument.productBrief=result(run,'product');finalDocument.marketingPlan=result(run,'marketing');finalDocument.creativeDirection=result(run,'creative');finalDocument.creativeDirectionAcceptance=await readJSON('creative-direction-acceptance.json').catch(()=>null);finalDocument.directorTimeline=result(run,'director');finalDocument.hyperframesDesignPlan=result(run,'hyperframes');finalDocument.catalogHash=discovery.data.contentHash;await saveJSON('document.json',finalDocument);}return finalDocument;
