...
export function commerceResourceContext(contract){
  const template=commerceTemplates[contract?.scenarioId];
...
export function commerceComponentReceipt(document){
  ...
  const instances=document.nodes.flatMap(n=>{
    let componentId=['video','image'].includes(n.kind)?'hero_media':n.semanticRole==='title'?'title':n.semanticRole==='caption'?'caption':n.semanticRole==='cta'?'ending':n.semanticRole==='feature'?(document.businessContract.scenarioId==='product_howto'?'operation_step':'detail_callout'):null;
