...
export async function measure(job,name,work,details={}){
  ...
  try{const value=await work();const cacheHit=value?.cacheHit??value?.metrics?.cacheHit??(name==='prepare_asset'||name==='prepare_voice'?value?.preparationCacheHit:name==='prepare_analysis'?value?.analysisCacheHit:name==='prepare_speech'?value?.speechPreparationCacheHit:undefined);if(cacheHit)job.metrics.cacheHits++;const extraModelCalls=name==='analyze'?value?.analysisMetrics?.modelCalls||0:name==='quality_review'?value?.metrics?.modelCalls||0:0;job.metrics.modelCalls+=extraModelCalls;job.metrics.stages.push({name,startedAt,elapsedMs:Math.round(performance.now()-start),status:'complete',...(extraModelCalls?{modelCalls:extraModelCalls}:{}),...(cacheHit!==undefined?{cacheHit}:{}),...(value?.cache?{cache:value.cache}:{}),...details});return value;}
