...
export class AgentKernel {
  ...
  async run(run,{signal}={}){
    ...
    const execute=async()=>{
      ...
      const recordModelCall=async meta=>{signal?.throwIfAborted();if(run.modelCalls>=this.maxModelCalls)throw Object.assign(Error('模型调用预算已用完，检查点已保留'),{code:'MODEL_BUDGET'});run.modelCalls++;(run.modelInvocations??=[]).push({number:run.modelCalls,...meta,startedAt:new Date().toISOString()});await persist();};
      try{
