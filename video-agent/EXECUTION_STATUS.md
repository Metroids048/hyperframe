...
## 2026-09-19 用户复现后的链路修复（进行中）

- 真实复现确认：素材上传请求可写入工程 `uploads/`；原问题在消息接口同步等待路由/模型，前端一直停留在提交状态，导致用户感知为对话卡住且没有视频。
- 已修改 `lib/creative/service.mjs`：`action=message` 立即返回 `202 accepted`，路由、排队和失败回执在后台执行；前端已有工程轮询会继续读取真实 job、revision 和渲染百分比。
- 已重启 3024 服务并实测：消息请求在 0.01 秒内返回，工程随后显示 `running / 理解本次要求`；尚未把运行中状态宣称为成片完成。
- 当前仍待验证：后台路由完成后是否进入生成/渲染并产生 MP4；需要继续轮询至 `complete + revisionId + 可访问 MP4`，并补充上传、慢对话和成片三项回归证据后才能结束。
