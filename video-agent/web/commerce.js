...
async function listProjects(){
 ...
 const listedSources=new Set(visible.map(p=>p.id));
 const savedPresets=presetCatalog.filter(p=>!listedSources.has(p.sourceProjectId));
 $('#projects').replaceChildren(new Option('已导出作品',''),...savedPresets.map(p=>new Option(p.title,'preset:'+p.id)),...visible.map(p=>new Option(presetCatalog.find(example=>example.sourceProjectId===p.id)?.title||p.title||'未命名作品',p.id)));$('#projects').value=project?.id||'';
}
...
async function openProject(id){const sequence=++openSequence;opening=true;clearTimeout(poll);project=null;input.value='';files=[];thumbnails();clearPreview();draw();try{const result=await api('/api/commerce/'+id);if(sequence!==openSequence)return;project=result.project;restoreIntake();restoreMaterialSelection();selected=project.currentRevisionId;history.replaceState(null,'','/?project='+id);draw();await listProjects();await refresh();}finally{if(sequence===openSequence){opening=false;draw();}}}
...
function restoreIntake(){const r=project?.request||{},revision=project?.revisions?.find(v=>v.id===project.currentRevisionId),o=revision?.output||r.output||{};$('#creation-target').value=r.target||'marketing';$('#output-aspect').value=o.width===o.height?'1:1':o.width>o.height?'16:9':'9:16';$('#output-duration').value=revision?revision.durationFrames/30:o.durationSeconds||24;$('#business-scene').value=r.scenarioId||'auto';selectedBusinessGoal=r.scenarioId==='product_launch'?'launch':['product_demo','product_howto'].includes(r.scenarioId)?'demo':'auto';}
function intake(){const ratio=$('#output-aspect').value;return {target:$('#creation-target').value,scenarioId:selectedBusinessGoal==='demo'?'product_demo':selectedBusinessGoal==='launch'?'product_launch':null,businessGoal:['launch','demo'].includes(selectedBusinessGoal)?[selectedBusinessGoal]:[],output:{width:ratio==='16:9'?1920:1080,height:ratio==='9:16'?1920:1080,durationSeconds:Number($('#output-duration').value)},pipelineVersion:3};}
composer.onsubmit=async e=>{e.preventDefault();if(busy())return;const message=input.value.trim();if(!message&&!files.length)return;$('#error').hidden=true;posting=true;draw();try{if(project?.currentRevisionId){await uploadFiles();if(await action({action:'patch',message,selectedNodeId:selectedNode})){input.value='';files=[];thumbnails();}}else{if(!project)project=(await post({action:'draft',request:{message,inferRequest:true,...intake()}})).project;selected=null;rememberProject(project.id);history.replaceState(null,'','/?project='+project.id);draw();if($('#material-root').value)project=(await post({action:'material-root',projectId:project.id,materialRootId:$('#material-root').value})).project;await uploadFiles();await post({action:'generate',projectId:project.id,message,request:intake(),idempotencyKey:crypto.randomUUID()});input.value='';files=[];thumbnails();await refresh();await listProjects();}}catch(err){showError(err);}finally{posting=false;draw();}};
...
$('#new-project').onclick=()=>{location.href='/';};$('#projects').onchange=e=>{if(e.target.value==='__toggle_all__'){showAllProjects=!showAllProjects;void listProjects();}else if(e.target.value.startsWith('preset:')){const preset=presetCatalog.find(p=>p.id===e.target.value.slice(7));if(preset)void loadPreset(preset);}else if(e.target.value)openProject(e.target.value).catch(showError);};$('#objects').onchange=e=>chooseObject(e.target.value,true);$('#revisions').onchange=e=>{selected=e.target.value;draw();};$('#restore').onclick=()=>action({action:'restore',revisionId:selected});$('#undo').onclick=()=>action({action:'undo'});$('#redo').onclick=()=>action({action:'redo'});$('#export').onclick=()=>action({action:'export',revisionId:selected});
...
async function refreshExamples(){
 try{const data=await api('/api/commerce-demos'),next=data.presets.filter(p=>p.category&&p.category!=='基础示例'),signature=JSON.stringify(next);drawBusinessCases(next.filter(p=>!p.featuredWork));if(signature!==JSON.stringify(presetCatalog)){const previous=chosenPreset?.id||new URLSearchParams(location.search).get('demo');presetCatalog=next;$('#example-select').replaceChildren(new Option('选择一个商品场景',''),...presetCatalog.map(p=>new Option(p.title+' · '+p.category,p.id)));if(presetCatalog.length)chooseExample(presetCatalog.some(p=>p.id===previous)?previous:project?.preset?.id||'',!project);shownEvidence='';drawExampleEvidence();void listProjects();if(project&&next.some(p=>p.sourceProjectId===project.id))void refresh().catch(showError);}$('#examples-loading').hidden=Boolean(presetCatalog.length)&&!data.unavailable?.filter(p=>p.category!=='基础示例').length;$('#examples-loading').textContent=data.unavailable?.length?data.unavailable.filter(p=>p.category!=='基础示例').map(p=>p.title+'：'+p.reason).join('；'):'本轮示范尚未完成。可以先上传自己的素材开始创作。';}catch(error){if(!presetCatalog.length)$('#examples-loading').textContent='示例暂时无法读取，你仍可输入需求开始创作。';}
 setTimeout(refreshExamples,15000);
}
...
if(sceneSelect)sceneSelect.onchange=()=>{selectedBusinessGoal=sceneSelect.value==='product_demo'?'demo':sceneSelect.value==='product_launch'?'launch':null;};
function useCreationEntry(kind){
 const entry=creationInputs[kind];if(!entry)return;if(!['launch','demo'].includes(kind)){$('#input-guidance').textContent='此类历史样例可查看和续改；新制作请选择单品上新或操作演示。';return;}
...
let materialRoots=[];
api('/api/commerce-material-roots').then(data=>{materialRoots=data.roots;$('#material-root').replaceChildren(new Option('上传我的素材',''),...materialRoots.map(r=>new Option(r.label+' · '+r.videos+' 条视频 / '+r.images+' 张图片',r.id)));restoreMaterialSelection();}).catch(showError);
$('#material-root').onchange=()=>{const r=materialRoots.find(r=>r.id===$('#material-root').value);$('#material-count').textContent=r?'已发现 '+r.videos+' 条视频 / '+r.images+' 张图片，Agent 会观察并选择镜头。':'也可以上传自己的视频和图片。';};

function restoreMaterialSelection(){const saved=project?.request?.materialRoot;if(!saved)return;const option=[...$('#material-root').options].find(o=>o.value===saved.id);if(option)$('#material-root').value=saved.id;$('#material-count').textContent='当前工程素材：'+saved.label+' · '+project.assets.filter(a=>a.kind==='video').length+' 条视频 / '+project.assets.filter(a=>a.kind==='image').length+' 张图片';}
