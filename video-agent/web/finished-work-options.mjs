...
export function finishedWorkOptions(projects,presets,works){
  ...
  works=works.filter(w=>!library.archivedWorkIds.includes(w.id)||w.editable);
  if(library.hidePresets)presets=[];
  const boundProjects=new Set(works.filter(w=>w.editable).map(w=>w.projectId));
  const visible=projects.filter(p=>!boundProjects.has(p.id)&&p.visibility!=='test'&&p.testOnly!==true&&p.revisions?.some(r=>r.rendered&&r.videoUrl));
  ...
  return [
    ...works.map(w=>({value:'work:'+w.id,group:'我的视频',label:w.title+(w.provenance?.startsWith('reference-')?' · 参考成品':w.editable?' · 继续编辑':' · 原生工程未绑定')})),
